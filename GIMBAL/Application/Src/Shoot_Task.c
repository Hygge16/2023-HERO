...
int SHOOT_SPEED_16M_S = 6250,SHOOT_SPEED_10M_S = 4200;//ӫ�ⵯ����5560-5570����ʱ����5515
int Shoot_speed_mode = 0;
float speed_add,speed_tem;
int stuck_flag = 0;//����Ϊ0������Ϊ1
int stall_back_flag;
...
static void Wheel_Ctrl(void)
{
  ...
  float speed_Err[2] = {0,},target_speed;
	...
	else if(Chassis_Ctrl.ctrl == 2)//����ģʽ
  {
...
{
  float temp_scope = 15;//����仯��ΧΪ25���϶�
  float temp_low = 29;//��ʼ�¶��趨Ϊ35���϶�
  float res = 0;
  float temp_real;
...
static float SpeedAdapt_10M(float real_S , float min_S, float max_S,float up_num , float down_num)
{
  ...
  if(real_S < 8) res+=2*up_num;//����̫��
	else if(real_S < min_S && real_S > 8) res+=up_num;//����ƫ��
  else if(real_S >= min_S && real_S <= max_S )res = 0;
	else if(real_S > max_S) res -= down_num;//����ƫ��
...
static float SpeedAdapt_16M(float real_S , float min_S, float max_S,float up_num , float down_num)
{
  ...
  if(real_S < 14.3f) res+=1.8f*up_num;//����̫��
	else if(real_S < min_S && real_S > 14.3f) res+=up_num;//����ƫ��
  else if(real_S >= min_S && real_S <= max_S )res = 0;
	else if(real_S > max_S && real_S <15.5f) res -= down_num;//������΢ƫ��
...
{	
    ...
    int16_t send_Value;
      ...
      PID_Init_ByParamArray(&DJI_Motor[Trigger].pid_Angle, trigger_PID_Param[1][1]);
		...
		{
			if(Chassis_Ctrl.ctrl == 1 && rc_ctrl.rc.s[1] == 2)//ң��������
			{
			...
			else if((rc_ctrl.rc.ch[1] != 0 && RC_shoot_flag == 0))
				{
					...
					Shoot.trigger_Angle = DJI_Motor[Trigger].Data.angle	+ RC_direction * 186.f;//}12��31
					Trigger_time_num = 0;
					RC_shoot_flag = 1;
				}
		...
		else if(Chassis_Ctrl.ctrl == 2 && Shoot.shoot_mode == Shoot_On)//���̿��ƣ�����
		{
			if(Trigger_time_num < 300)
			{
				Trigger_time_num++;
			...
			else
			{
				...
				if(IF_TRIGGER_POSITIVE == true && Key_shoot_flag == 0)//�������򵯣���������ת
				{
					if(robot.shooter_id1_42mm.cooling_limit - robot.cooling_heat >= 100)//������ǹ����������
					{
				...
				if(IF_TRIGGER_NEGATIVE == true && Key_shoot_flag == 0)//����R�˵��������̷�ת
				{
