...
pm01_od_t pm01_od;
uint16_t  pm01_access_id;    /* 正在访问的标识符     */
uint16_t  pm01_response_flg; /* 控制器响应成功标志位 */

uint16_t g_cmd_set   = 2;
uint16_t g_power_set = 5000;//&game_robot_state.chassis_power_limit;
uint16_t g_power_set_offset=0;
uint16_t g_vout_set  = 2250;
...
void pm01_cmd_send( uint16_t new_cmd, uint8_t save_flg )
{
	...
	CAN_TxHeaderTypeDef  power_tx_message={0,};
		  ...
		  switch(i)
			{
					...
					case 9:
						  ...
						  pm01_power_set( g_power_set + g_power_set_offset, 0x00 );
					  break;
					case 10:
